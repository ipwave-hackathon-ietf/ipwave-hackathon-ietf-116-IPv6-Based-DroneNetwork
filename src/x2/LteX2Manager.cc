...
{
    std::cout <<"---- void LteX2Manager::initialize(int stage) ---- " << std::endl;
...
#ifdef IPv6Based
        std::cout <<"---- IPv6Based  ----- " <<std::endl;
        Ipv6FlatNetworkConfigurator* configurator = check_and_cast<Ipv6FlatNetworkConfigurator*>(getModuleByPath("configurator"));
//        IInterfaceTable *interfaceTable =  configurator->findInterfaceTableOf(getParentModule()->getParentModule());


#else
        std::cout << "------ IPv4Based ------ "<<std::endl;
