...
void IP2Nic::initialize(int stage)
{
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        std::cout<<"------------ IP2NIC runs! Module name  :  " << getParentModule()->getFullName()<< "Type : " << par("nodeType").stdstringValue() <<std::endl;
        ...
        std::cout<<"11111111111111111   :  "<< getParentModule()->getParentModule()->getFullName() << std::endl;
    }
...
{
    std::cout <<" -- IP2Nic::handleMessage(cMessage *msg)  ---" <<std::endl;
    if( nodeType_ == ENODEB || nodeType_ == GNODEB)
    {
...
{    std::cout << "Node type: " << s << " -> " << nodeType_ << endl;
...
{
    std::cout << "-- void IP2Nic::fromIpUe(Packet * datagram) -- " <<std::endl;
...
{
    std::cout <<" -- void IP2Nic::toStackUe(Packet * pkt) --" <<std::endl;
#ifdef IPv6Based
/*    auto ipHeader = pkt->peekAtFront<Ipv6Header>();
    auto srcAddr  = ipHeader->getSrcAddress();
    auto destAddr = ipHeader->getDestAddress();
    short int tc = ipHeader->getTrafficClass();//Traffic class serving as advanced tos
    int headerSize = ipHeader->getPayloadLength().get();
...
//     pkt->addTagIfAbsent<FlowControlInfo>()->setSrcAddr(srcAddr.srcAddr);
//     pkt->addTagIfAbsent<FlowControlInfo>()->setDstAddr(destAddr.getInt());
     pkt->addTagIfAbsent<FlowControlInfo>()->setHeaderSize(headerSize);
     pkt->addTagIfAbsent<FlowControlInfo>()->setTypeOfService(tc);
    if (transportProtocol == IP_PROT_TCP) {
        auto tcpHeader = pkt->peekDataAt<tcp::TcpHeader>(ipHeader->getChunkLength());
    }*/
    printIPv6BasedControlInfo(pkt);
#else
...
void IP2Nic::prepareForIpv4(Packet *datagram, const Protocol *protocol){
    ...
    std::cout <<"-- void IP2Nic::prepareForIpv4(Packet *datagram, const Protocol *protocol) -- " <<std::endl;
...
void IP2Nic::prepareForIpv6(Packet *datagram, const Protocol *protocol){
    // add DispatchProtocolRequest so that the packet is handled by the specified protocol
    std::cout <<"-- void IP2Nic::prepareForIpv6(Packet *datagram, const Protocol *protocol) -- " <<std::endl;
    datagram->addTagIfAbsent<DispatchProtocolReq>()->setProtocol(protocol);
    datagram->addTagIfAbsent<PacketProtocolTag>()->setProtocol(protocol);
    // add Interface-Indication to indicate which interface this packet was received from
    datagram->addTagIfAbsent<InterfaceInd>()->setInterfaceId(networkIf->getInterfaceId());
...
{
    std::cout <<"--void IP2Nic::toIpUe(Packet *pkt)  -- "<<std::endl;
#ifdef IPv6Based
    auto ipHeader = pkt->peekAtFront<Ipv6Header>();
...
{
    std::cout <<"-- void IP2Nic::fromIpBs(Packet * pkt) -- " <<std::endl;
    ...
    auto ipHeader = pkt->peekAtFront<Ipv6Header>();
    const Ipv6Address& destAddr = ipHeader->getDestAddress();
...
{
#ifdef IPv6Based
    std::cout <<"-- void IP2Nic::toIpBs(Packet* pkt) --" <<std::endl;
    auto ipHeader = pkt->peekAtFront<Ipv6Header>();
    auto networkProtocolInd = pkt->addTagIfAbsent<NetworkProtocolInd>();
    networkProtocolInd->setProtocol(&Protocol::ipv6);
    networkProtocolInd->setNetworkProtocolHeader(ipHeader);
    prepareForIpv4(pkt, &LteProtocol::ipv6uu);
    EV << "IPmessage from stack: send to IP layer" << endl;
    send(pkt,ipGateOut_);
#else
    std::cout <<"-- void IP2Nic::toIpBs(Packet* pkt) --" <<std::endl;
    auto ipHeader = pkt->peekAtFront<Ipv4Header>();
...
{
    std::cout <<"-- void IP2Nic::toStackBs(Packet* pkt) -- " <<std::endl;
...
//    if (ipv6Header->getDestAddress().isMulticast())
        ...
        if (ipHeader->getDestAddress().isMulticast()){
            std::cout <<"!!!!!!!!!!!!!!  MulticastPacket" << std::endl;
        }
//        short int tos = ipHeader->getTypeOfService();
        short int tc = ipHeader->getTrafficClass();//Traffic class serving as advanced tos
//        int headerSize = ipHeader->getHeaderLength().get();
        int headerSize = ipHeader->getExtensionHeaderArraySize();
       ...
       switch(transportProtocol)
        {
            ...
            default: {
                std::cout << "Unknown transport protocol id. transportProtocol = "<< transportProtocol<< endl;
                EV_ERROR << "Unknown transport protocol id." << endl;
            }
        }
       // prepare flow info for NIC
//       pkt->addTagIfAbsent<FlowControlInfo>()->setSrcAddr(srcAddr.getInt());
       pkt->addTagIfAbsent<FlowControlInfo>()->setSrcAddr(srcAddr.words()[0]);
//       pkt->addTagIfAbsent<FlowControlInfo>()->setDstAddr(destAddr.getInt());
       pkt->addTagIfAbsent<FlowControlInfo>()->setDstAddr(destAddr.words()[0]);
       pkt->addTagIfAbsent<FlowControlInfo>()->setTypeOfService(tc);
       pkt->addTagIfAbsent<FlowControlInfo>()->setHeaderSize(headerSize);
        // mark packet for using NR
        if (!markPacket(pkt->getTagForUpdate<FlowControlInfo>()))
        ...
        {
            EV << "----- Issues Now Marked! print and send! ------" << endl;
            printControlInfo(pkt);
            send(pkt,stackGateOut_);
        }
#else
...
{
    std::cout <<"--  void IP2Nic::printControlInfo(Packet* pkt) -- " <<std::endl;
...
void IP2Nic::printIPv6BasedControlInfo(inet::Packet* pkt){
    std::cout <<"--  void IP2Nic::printIPv6BasedControlInfo(Packet* pkt) -- " <<std::endl;
//     EV << "Src IP : " << Ipv6Address(pkt->getTag<FlowControlInfo>()->getSrcAddr()) << endl;
//     EV << "Dst IP : " << Ipv6Address(pkt->getTag<FlowControlInfo>()->getDstAddr()) << endl;
     EV << "ToS : " << pkt->getTag<FlowControlInfo>()->getTypeOfService() << endl;
     EV << "Seq Num  : " << pkt->getTag<FlowControlInfo>()->getSequenceNumber() << endl;
     EV << "Header Size : " << pkt->getTag<FlowControlInfo>()->getHeaderSize() << endl;
}
...
{
    std::cout <<"-- void IP2Nic::registerInterface()--"<<std::endl;
...
{
    std::cout <<"-- void IP2Nic::registerMulticastGroups() -- "<<std::endl;
...
#ifdef IPv6Based
//    unsigned int numOfAddresses = iface->getProtocolData<Ipv6InterfaceData>()->get;
#else
...
bool IP2Nic::markPacket(inet::Ptr<FlowControlInfo> ci)
{
    ...
    std::cout <<"-- bool IP2Nic::markPacket(inet::Ptr<FlowControlInfo> ci) -- "<<std::endl;

    if (nodeType_ == ENODEB || nodeType_ == GNODEB)
    {
#ifdef IPv6Based
//        MacNodeId ueId = binder_->getMacNodeId((Ipv6Address)ci->getDstAddr());
//        MacNodeId nrUeId = binder_->getNrMacNodeId((Ipv6Address)ci->getDstAddr());
...
{
    std::cout <<"--  void IP2Nic::triggerHandoverSource(MacNodeId ueId, MacNodeId targetEnb) --" <<std::endl;
...
{
    std::cout << NOW << " IP2Nic::triggerHandoverTarget - start holding packets destined to " << ueId << endl;
...
{
    std::cout << "-- void IP2Nic::sendTunneledPacketOnHandover(Packet* datagram, MacNodeId targetEnb)--" <<std::endl;
...
{
    std::cout << "IP2lte::receiveTunneledPacketOnHandover - received packet via X2 from " << sourceEnb << endl;
    EV << "IP2lte::receiveTunneledPacketOnHandover - received packet via X2 from " << sourceEnb << endl;
#ifdef IPv6Based
    const auto& hdr = datagram->peekAtFront<Ipv6Header>();
#else
...
{
    std::cout <<"-- void IP2Nic::signalHandoverCompleteTarget(MacNodeId ueId, MacNodeId sourceEnb) -- " <<std::endl;
...
{
    std::cout<< NOW << " IP2Nic::triggerHandoverUe - start holding packets" << endl;
...
{
    cout <<"-- void IP2Nic::signalHandoverCompleteUe(bool isNr) -- " <<std::endl;
